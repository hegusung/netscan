...
class LDAPScan:
    ...
    def list_users(self, callback):
        ...
        def process(item):
            ...
            fullname = str(attr['displayName']) if 'displayName' in attr and len(attr['displayName']) > 0 else ""
            ...
            if 'adminCount' in attr and len(attr['adminCount']) != 0 and int(str(attr['adminCount'])) > 0:
        ...
        sc2 = ldapasn1.SDFlagsControl(criticality=True, flags=0x5)
        ...
        self.conn.search(searchBase=sbase, searchFilter='(|(objectCategory=user)(objectCategory=CN=ms-DS-Group-Managed-Service-Account,CN=Schema,CN=Configuration,DC=main,DC=local))', searchControls=[sc, sc2], attributes=attributes, perRecordCallback=process)
